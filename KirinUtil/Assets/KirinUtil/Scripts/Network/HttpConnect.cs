...
namespace KirinUtil {
    public class HttpConnect:MonoBehaviour {
        ...
        */
        ...
        //----------------------------------
        #region SendImage
        public static void SendImage(string url, Texture2D texture, string fileName, Action<UnityWebRequest> onSuccess, Action<UnityWebRequest> onError = null) {

            Instance.StartCoroutine(Instance.WaitSendImage(url, texture, fileName, onSuccess, onError));

        }

        private IEnumerator WaitSendImage(string url, Texture2D texture, string fileName, Action<UnityWebRequest> onSuccess, Action<UnityWebRequest> onError) {
            ...
            WWWForm form = new WWWForm();
            form.AddBinaryData("file", imageData, fileName, type);
